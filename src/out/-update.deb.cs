...
void main(string[] args)
{
    ...
    {
        Console.WriteLine("Uninstalling package: 'sudo dpkg --purge cs-script.core'");
        Process.Start("dpkg", "--purge cs-script.core")
               .WaitForExit();
    }
    else if (args.FirstOrDefault() == "-i" || args.Count() == 0)
    {
        ...
        else
        {
            ...
            var packageUrl = rootUrl + "/cs-script.core_" + version + "_all.deb";
            Console.WriteLine("Downloading " + packageUrl + " ...");

            // install: sodo dpkg -i cs-script.core_3.27.2.1.deb
            // uninstall: sudo dpkg --purge cs-script.core
